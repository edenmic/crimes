...
// Add these mappings to your existing offenseMap object

const offenseMap: Record<string, string> = {
  // Your existing mappings...
  
  // Add these new mappings
  'Verbal Disputes': 'Verbal Dispute',
  'Medical Assistance': 'Medical Incident',
  'Drug Violation': 'Drug Related',
  'Robbery': 'Robbery',
  'Missing Person Located': 'Person Found',
  'Missing Person Reported': 'Missing Person',
  'Fire Related Reports': 'Fire Incident',
  'Firearm Discovery': 'Weapon Found',
  'Commercial Burglary': 'Burglary',
  'Offenses Against Child / Family': 'Family Offense',
  'Police Service Incidents': 'Police Service',
  'Search Warrants': 'Warrant Execution',
  'Operating Under the Influence': 'OUI/DUI',
  'Landlord/Tenant Disputes': 'Civil Dispute',
  'Ballistics': 'Firearm Related',
  'Prisoner Related Incidents': 'Custody Incident',
  'License Plate Related Incidents': 'Vehicle Registration',
  'Harbor Related Incidents': 'Harbor Incident',
  'Prostitution': 'Prostitution',
  'HOME INVASION': 'Home Invasion',
  'Homicide': 'Homicide'
};

// Then modify your mapping function to include a fallback
function mapOffense(offense: string): string {
  if (offenseMap[offense]) {
    return offenseMap[offense];
  }
  
  console.warn(`Unmapped offense: ${offense}`);
  return 'Other'; // Default category for unmapped offenses
}
